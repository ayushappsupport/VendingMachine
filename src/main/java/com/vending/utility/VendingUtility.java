			...
			if (validateCoin(coin)) {
				if (IntStream.of(Coin.POSSIBLE_VALUES).anyMatch(x -> x == coin.getDenomination())) {
					if (!coinPresentMap.containsKey(coin.getDenomination())) {
						machine.setCurrentAmount(
								machine.getCurrentAmount() + (coin.getDenomination() * coin.getCount()));
						resultCoins.add(vendingServiceData
								.saveCoin(new Coin(machine, coin.getDenomination(), coin.getCount())));
					} else {
						machine.setCurrentAmount(
								machine.getCurrentAmount() + (coin.getDenomination() * coin.getCount()));
